...
function getSession(req){return sessions.get(req.headers["x-telaflux-session"]||req.headers["x-cineverse-session"])}function sessionToken(){return crypto.randomBytes(32).toString("hex")}
...
app.get("/api/auth/start",async(req,res)=>{try{if(!TMDB_TOKEN)throw new Error("TMDB_TOKEN não configurado");const r=await fetch("https://api.themoviedb.org/3/authentication/token/new",{headers:{Authorization:`Bearer ${TMDB_TOKEN}`}}),d=await r.json();if(!d.success)throw new Error(d.status_message||"Falha");res.redirect(`https://www.themoviedb.org/authenticate/${d.request_token}?redirect_to=${encodeURIComponent(API_PUBLIC_URL+"/api/auth/callback")}`)}catch(e){res.status(500).send(`<h2>Erro de login</h2><p>${e.message}</p>`)}});
app.get("/api/auth/callback",async(req,res)=>{try{const r=await fetch("https://api.themoviedb.org/3/authentication/session/new",{method:"POST",headers:{Authorization:`Bearer ${TMDB_TOKEN}`,"Content-Type":"application/json"},body:JSON.stringify({request_token:req.query.request_token})}),d=await r.json();if(!d.success)throw new Error(d.status_message||"Autenticação recusada");const me=await tmdb("account",{session_id:d.session_id}),browserToken=sessionToken();sessions.set(browserToken,{session_id:d.session_id,account:me});res.redirect(`${FRONTEND_URL||API_PUBLIC_URL}/?telaflux_session=${encodeURIComponent(browserToken)}`)}catch(e){res.status(500).send(`<h2>Não foi possível concluir o login</h2><p>${e.message}</p><a href="/">Voltar</a>`)}});
app.get("/api/me",async(req,res)=>{const s=getSession(req);if(!s)return res.status(401).json({authenticated:false});res.json({authenticated:true,account:s.account})});app.post("/api/logout",(req,res)=>{sessions.delete(req.headers["x-telaflux-session"]||req.headers["x-cineverse-session"]);res.json({ok:true})});
...
app.get("/api/upcoming",async(req,res)=>{
  try{
    ...
    const enriched=await Promise.all(base.map(async x=>{
      ...
      let platform="",trailer_key="";
      try{
        const ps=await getProviders(x.media_type,x.id);
        platform=ps.slice(0,2).join(" · ");
        if(x.release_kind==="Digital"&&ps.length)x.release_kind="Streaming";
      }catch{}
      try{
        const v=await tmdb(`${x.media_type}/${x.id}/videos`,{language:"pt-BR"});
        const tr=(v.results||[]).find(z=>z.site==="YouTube"&&(z.type==="Trailer"||z.type==="Teaser"));
        trailer_key=tr?.key||"";
      }catch{}
      const project=scoreProject(detail,credits);
      return {...x,...detail,media_type:x.media_type,release_date:x.release_date,release_kind:x.release_kind,platform,trailer_key,project_score:project.score,project_signals:project.signals,director:project.director};
    }));

    const major=enriched
      .filter(x=>x.project_score>=45)
    ...
    const data={results:[...major,...discoveries],major,discoveries,updated_at:new Date().toISOString(),period:{from:today,to:end},classification:{major_threshold:45,notes:"Classificação automática por estúdio, franquia, diretor, elenco, popularidade e votos."}};
    upcomingCache.data=data;upcomingCache.expires=Date.now()+300000;
    res.json(data);
  }catch(e){console.error("upcoming:",e);res.status(500).json({error:e.message})}
});
