...
const app=express(),PORT=process.env.PORT||3000,TMDB_TOKEN=process.env.TMDB_TOKEN,TMDB_API_KEY=process.env.TMDB_API_KEY,APP_URL=process.env.APP_URL||process.env.API_URL||process.env.RENDER_EXTERNAL_URL||`http://localhost:${process.env.PORT||3000}`,FRONTEND_URL=(process.env.FRONTEND_URL||"https://marcosvivendo6-ux.github.io/telaflux").replace(/\/+$/,"");const FRONTEND_ORIGIN=(()=>{try{return FRONTEND_URL?new URL(FRONTEND_URL).origin:""}catch{return""}})(),API_PUBLIC_URL=(process.env.API_URL||process.env.RENDER_EXTERNAL_URL||APP_URL).replace(/\/+$/,"");const sessions=new Map(),providerCache=new Map(),upcomingCache={expires:0,data:null},discoverCache=new Map();
...
app.get("/api/cinema-month",async(req,res)=>{
  try{
    ...
    const month=Number(new Intl.DateTimeFormat("en-US",{timeZone:"America/Sao_Paulo",month:"2-digit"}).format(now));
    const key=`cinema-month:${year}-${String(month).padStart(2,"0")}`;
    const cached=upcomingCache[key];
    if(cached&&cached.expires>Date.now())return res.json(cached.data);
    const from=`${year}-${String(month).padStart(2,"0")}-01`;
    const lastDay=new Date(Date.UTC(year,month,0)).getUTCDate();
    const to=`${year}-${String(month).padStart(2,"0")}-${String(lastDay).padStart(2,"0")}`;
    const pages=await Promise.all([1,2,3].map(page=>tmdb("discover/movie",{language:"pt-BR",region:"BR","primary_release_date.gte":from,"primary_release_date.lte":to,sort_by:"primary_release_date.asc",with_release_type:"2|3",page})));
    const map=new Map();
    for(const page of pages){for(const x of (page?.results||[])){if(x?.id&&!map.has(x.id))map.set(x.id,x);}}
    const results=[...map.values()].filter(x=>x?.release_date&&x.release_date>=from&&x.release_date<=to).sort((a,b)=>String(a.release_date||"").localeCompare(String(b.release_date||""))||Number(b.popularity||0)-Number(a.popularity||0)).slice(0,30);
    const data={results,updated_at:new Date().toISOString(),period:{year,month,from,to}};
    upcomingCache[key]={data,expires:Date.now()+600000};
    res.setHeader("Cache-Control","public, max-age=300, stale-while-revalidate=600");
    res.json(data);
  }catch(e){console.error("cinema-month:",e);res.status(500).json({error:"internal_error"})}
});
...
app.get("/api/discover",async(req,res)=>{try{
  ...
  const ratingRows=db.prepare("SELECT tmdb_id,media_type,ROUND(AVG(rating),1) AS average,COUNT(*) AS votes FROM ratings GROUP BY tmdb_id,media_type").all();const ratingMap=new Map(ratingRows.map(r=>[`${r.media_type}:${r.tmdb_id}`,{average:Number(r.average),votes:Number(r.votes)}]));for(const item of combinedResults){const rr=ratingMap.get(`${item.media_type}:${item.id}`);item.user_rating=rr?.average??null;item.user_rating_votes=rr?.votes??0}
